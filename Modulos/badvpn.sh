...
if ps x | grep "udpvpn"|grep -v grep 1>/dev/null 2>/dev/null; then
   echo ""
   echo -e "\033[1;32mO BADVPN YA ESTA ACTIVO !\033[0m"
   sleep 3
   badvpn
 else
   echo ""
   ...
   fun_bar 'fun_udpon'
   echo ""
   echo -e "   \033[1;32mBADVPN ACTIVO !\033[0m"
   sleep 3
   badvpn
fi
}
...
echo ""
 if ps x | grep "udpvpn"|grep -v grep 1>/dev/null 2>/dev/null; then
   ...
   echo ""
   echo -e " \033[1;32mO BADVPN YA ESTA DESACTIVADO!\033[0m"
   sleep 3
   badvpn
 fi
}
...
statusudp="\033[1;31mDESACTIVADO"
fi
clear
...
if [[ "$resposta" = 's' ]]; then
   ...
   chmod 777 /bin/badvpn-udpgw
	...
	fun_bar 'inst_udp'
	echo ""
	echo -e "\033[1;32mBADVPN INSTALADO CON ÉXITO."
	sleep 3
	badvpn
else 
   echo ""
